...
import { Inbox, Package, ArrowRight, Loader2, RefreshCw, UserPlus, User } from 'lucide-react';
    ...
    const fetchData = async () => {
        ...
        try {
            const [analyticsRes, usersRes] = await Promise.all([
                ...
                isAdmin ? api.get('/admin/users?role=warehouse_op') : Promise.resolve({ data: { data: [] } })
            ]);
            setItems(analyticsRes.data.data.incomingQueue);
    ...
    return (
        <div className="space-y-10 pb-20">
            <header className="flex justify-between items-center">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-2">Incoming Reception</h1>
                    <p className="text-slate-500 font-medium">Packages currently at hub awaiting sortation and assignment.</p>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50/80 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                            <tr>
                                ...
                                <th className="px-10 py-6">Current Handler</th>
                                ...
                                <tr key={s._id} className="hover:bg-slate-50/50 transition duration-300">
                                    <td className="px-10 py-7">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-blue-50 text-blue-600 border border-blue-100">
                                                {s.status?.replace(/_/g, ' ')}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-10 py-7">
                                        <div className="font-bold text-slate-700 text-sm">{s.sender?.city}</div>
                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{s.sender?.country}</div>
                                    </td>
                                    <td className="px-10 py-7">
                                        <div className="font-mono text-sm text-slate-500 font-bold">{s.totalWeight} KG</div>
                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{s.type}</div>
                                    </td>
                                    <td className="px-10 py-7">
                                        {isAdmin ? (
                                            assigningId === s._id ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-primary hover:bg-primary/5 px-3 py-2 rounded-xl transition"
                                                >
                                                    {s.assignedTo ? (
                                                        <>
                                    ...
                                    <td className="px-10 py-7 text-right">
                                        <Link to={`/dashboard/shipments/${s._id}`} className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-slate-700 hover:scale-105 transition-all active:scale-95 shadow-lg shadow-slate-900/10">
